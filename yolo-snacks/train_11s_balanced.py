"""
YOLO11s + 类别均衡过采样 + 强增强（GPU 训练，部署口径模型）。
逻辑与 train_balanced.py 完全一致，仅换模型 yolo11s.pt、batch=32。
- 过采样：按"图片中最稀有类"决定复制倍数，写入图片列表 txt（列表重复行即过采样，不复制文件）
...
EPOCHS = 100
IMGSZ = 640
BATCH = 32        # YOLO11s@640 很小，batch=32 更充分利用 12G 显存
DEVICE = 0        # GPU
WORKERS = 0       # Windows 下配 cache=True 避免多进程 dataloader 卡死
NAME = "snacks_11s_balanced"
...
def main():
    ...
    print("Model=%s batch=%d imgsz=%d epochs=%d device=%s" % (MODEL, BATCH, IMGSZ, EPOCHS, DEVICE))

    model = YOLO(MODEL)
    model.train(
        data=dy, epochs=EPOCHS, imgsz=IMGSZ, batch=BATCH, device=DEVICE,
        workers=WORKERS, cache=True,
        project=r"E:\Code\Yueqian\yolo-snacks\runs\detect\runs", name=NAME, patience=30,
        # 强增强（提升稀有类泛化）
